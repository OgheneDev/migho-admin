...
const SignInPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Access the tokens from the nested structure
      ...
      console.log("Access Token:", accessToken);  // Optional: Log access token to verify
      console.log("Refresh Token:", refreshToken);  // Optional: Log refresh token to verify
  ...
  return (
    <div className='form-container flex flex-col-reverse w-full md:flex-row-reverse md:h-screen'>
      <form onSubmit={handleSubmit} className='p-10 flex flex-col justify-center md:w-1/2 bg-white'>
        ...
        <button type='submit' className='bg-custom-orange text-white px-10 py-3 w-full font-sans uppercase rounded-full md:w-fit mx-auto'>
      ...
      <div className="welcome bg-custom-orange text-white text-center w-full p-8 h-auto flex flex-col justify-center md:w-1/2 hidden md:block md:flex">
        <article>
          ...
          <p className='mb-7 font-sans md:text-lg'>Enter your administrator details to start <br />and manage your site.</p>
        </article>
      </div>
    </div>
  );
